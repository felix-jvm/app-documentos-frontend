...
export default function Presupuesto (props) {
 ...
 useEffect(() => {
  setTimeout(() => {
    if(props.formsData.current['specificData'] && props.formsData.current['specificData']['CategorizacionGasto']) {
     let tHead = document.getElementsByClassName('PresupuestoHead')[0]
     let tBody = document.getElementsByClassName('PresupuestoBody')[0]
     for(let records of props.formsData.current['specificData']['CategorizacionGasto']) {
      let columnSchema = ['Descripción','Sigla']
      let trBody = document.createElement('tr')
      trBody.className = 'PresupuestoTr'
      ...
      let trList = document.getElementsByClassName('PresupuestoTr') 
     ...
     let RendicionCuenta_PresupuestoDescrihtmlElem = document.getElementsByClassName('Presupuesto_DescripcionGeneral')[0] 
     let RendicionCuenta_PresupuestoSecondDescrihtmlElem = document.getElementsByClassName('Presupuesto_SegundaDescripcionGeneral')[0]
     RendicionCuenta_PresupuestoDescrihtmlElem.innerText = props.formsData.current['specificData']['Manual']['PresupuestoDescri']     
     RendicionCuenta_PresupuestoSecondDescrihtmlElem.innerText = props.formsData.current['specificData']['Manual']['PresupuestoSecondDescri']
    }  
     updateDeleteRecordButtonStatus()
    },250)},[])

 function updateDeleteRecordButtonStatus() {
  let PresupuestoBody = document.getElementsByClassName('PresupuestoBody')[0]
  setDisplayDeleteRecordButton(PresupuestoBody && PresupuestoBody.children.length? true:false)
 }
 ...
 function HandleAdd() {
  ...
  let PresupuestoHead = document.getElementsByClassName('PresupuestoHead')[0]    
  let PresupuestoBody = document.getElementsByClassName('PresupuestoBody')[0]    
  let data = [PresupuestoVariableDescripcion,PresupuestoVariableSigla]
  ...
  //  let parsedOptionValue = JSON.parse(Presupuesto_VariableDescripcion.value)
   props.backenData.current['CategorizacionGasto'].push({'Descri':PresupuestoVariableDescripcion.value,'Sigla':PresupuestoVariableSigla.value,'elementHtml':trBody.innerHTML});
   props.summaryData.current['CategorizacionGasto'][trBody.innerHTML] = {'Descri':PresupuestoVariableDescripcion.value,'Sigla':PresupuestoVariableSigla.value}      
   PresupuestoVariableDescripcion.value = '';
   PresupuestoVariableSigla.value = '';
  }
  PresupuestoBody.appendChild(trBody)
  !PresupuestoHead.children.length?PresupuestoHead.appendChild(trHead):void 0
  updateDeleteRecordButtonStatus()
 }
 ...
 function handleRecordRemove(){
  ...
  if(Object.keys(selectedTableRecord.current).includes('recordToDeleteId')){
   props.backenData.current['recordsToDelete'].push({'CategorizacionGasto':selectedTableRecord.current['recordToDeleteId']})
   Object.keys(props.summaryData.current['recordsToDelete']).includes('CategorizacionGasto')? props.summaryData.current['recordsToDelete']['CategorizacionGasto'].push(selectedTableRecord.current['record']):props.summaryData.current['recordsToDelete']['CategorizacionGasto']=[selectedTableRecord.current['record']]   
  }else{
    if(selectedTableRecord.current['record']){
      Object.keys(props.summaryData.current['CategorizacionGasto']).includes(selectedTableRecord.current['record'].innerHTML)? (()=>{delete props.summaryData.current['CategorizacionGasto'][selectedTableRecord.current['record'].innerHTML]})():void 0      
     for(let counter=0;counter<=props.backenData.current['CategorizacionGasto'].length-1;counter++){
      let currentRecordToCreate = props.backenData.current['CategorizacionGasto'][counter]
      if(currentRecordToCreate['elementHtml']==selectedTableRecord.current['record'].innerHTML){props.backenData.current['CategorizacionGasto'].splice(counter,1)}
    }}}
  ...
  useEffect((() => {  
    if(props.fullManualData) {
     let DescripcionGeneral = document.getElementsByClassName('Presupuesto_DescripcionGeneral')[0]    
     let SegundaDescripcionGeneral = document.getElementsByClassName('Presupuesto_SegundaDescripcionGeneral')[0]
     if(DescripcionGeneral.value){props.backenData.current['Manual']['PresupuestoDescri']=DescripcionGeneral.value}
