...
export default function BoundManual (props) {
 ...
 useEffect(() => {
  setTimeout(() => {
    let toFill = ['BoundManualesManual']
    ...
    for(let record of props.formsData.current[elemName]) {
     ...
     optionInnerText = `${record['Codigo']} __ ${record['ObjetivoGeneralManualDescri']}`
     option.innerText = optionInnerText    
     htmlElem? htmlElem.appendChild(option):void 0    
     htmlElem.value = ''
    } }
    if(props.formsData.current['specificData'] && props.formsData.current['specificData']['BoundManual']) {
     let tHead = document.getElementsByClassName('BoundManualesHead')[0]
     let tBody = document.getElementsByClassName('BoundManualesBody')[0]
     for(let records of props.formsData.current['specificData']['BoundManual']) {
      let columnSchema = ['Nomenclatura','Referencia del documento']
      let trBody = document.createElement('tr')
      trBody.className = 'BoundManualesTr'
      ...
      let trList = document.getElementsByClassName('BoundManualesTr') 
 ...
 function updateDeleteRecordButtonStatus() {
  let BoundManualesBody = document.getElementsByClassName('BoundManualesBody')[0]
  setDisplayDeleteRecordButton(BoundManualesBody && BoundManualesBody.children.length? true:false)
 }

 function HandleAdd() {
  let boundManualesManual = document.getElementsByClassName('BoundManualesManual')[0]
  if(boundManualesManual.selectedIndex==-1){return}
  let boundManualesManualValue = boundManualesManual.children[boundManualesManual.selectedIndex].innerText.split(' __ ')
  let BoundManualesHead = document.getElementsByClassName('BoundManualesHead')[0]    
  let BoundManualesBody = document.getElementsByClassName('BoundManualesBody')[0]    
  ...
  //  let parsedOptionValue = JSON.parse(MarcoLegalRegulatorioDescripción.value)
   props.backenData.current['BoundManual'].push({'Codigo':boundManualesManualValue[0],'Descri':boundManualesManualValue[1],'elementHtml':trBody.innerHTML});
   props.summaryData.current['BoundManual'][trBody.innerHTML] = {'Codigo':boundManualesManualValue[0],'Descri':boundManualesManualValue[1]}      
   boundManualesManual.value = '';
  }
  BoundManualesBody.appendChild(trBody)
  !BoundManualesHead.children.length?BoundManualesHead.appendChild(trHead):void 0
  updateDeleteRecordButtonStatus()
 }
 ...
 function handleRecordRemove(){
  ...
  if(Object.keys(selectedTableRecord.current).includes('recordToDeleteId')){
   props.backenData.current['recordsToDelete'].push({'BoundManual':selectedTableRecord.current['recordToDeleteId']})
   Object.keys(props.summaryData.current['recordsToDelete']).includes('BoundManual')? props.summaryData.current['recordsToDelete']['BoundManual'].push(selectedTableRecord.current['record']):props.summaryData.current['recordsToDelete']['BoundManual']=[selectedTableRecord.current['record']]   
  }else{
    if(selectedTableRecord.current['record']){
      Object.keys(props.summaryData.current['BoundManual']).includes(selectedTableRecord.current['record'].innerHTML)? (()=>{delete props.summaryData.current['BoundManual'][selectedTableRecord.current['record'].innerHTML]})():void 0      
     for(let counter=0;counter<=props.backenData.current['BoundManual'].length-1;counter++){
      let currentRecordToCreate = props.backenData.current['BoundManual'][counter]
      if(currentRecordToCreate['elementHtml']==selectedTableRecord.current['record'].innerHTML){props.backenData.current['BoundManual'].splice(counter,1)}
    }}}
