...
export default function ClienteExterno (props) {
 ...
 var selectedTableRecord = useRef(undefined);
  ...
  useEffect(() => {
    setTimeout(() => { 
      if(props.formsData.current['specificData'] && props.formsData.current['specificData']['ClienteExterno']) {
       let tHead = document.getElementsByClassName('ClientesExternosHead')[0]
       let tBody = document.getElementsByClassName('ClienteExternoBody')[0]
       for(let records of props.formsData.current['specificData']['ClienteExterno']) {
  ...
  function updateDeleteRecordButtonStatus() {
        let ClienteExternoBody = document.getElementsByClassName('ClienteExternoBody')[0]
        setDisplayDeleteRecordButton(ClienteExternoBody && ClienteExternoBody.children.length? true:false)
  }

  function HandleAdd() {
    let cliente = document.getElementsByClassName('ClienteExterno_Cliente')[0]
    let necesidad = document.getElementsByClassName('ClienteExterno_Necesidad')[0]   
    let expectativa = document.getElementsByClassName('ClienteExterno_Expectativa')[0]
    let ClientesExternosHead = document.getElementsByClassName('ClientesExternosHead')[0]    
    let ClienteExternoBody = document.getElementsByClassName('ClienteExternoBody')[0]    
    let data = [cliente,necesidad,expectativa]
    let columns = ['Cliente','Necesidad','Expectativa']
    var trHead = document.createElement('tr')
    var trBody = document.createElement('tr')
    trBody.className = 'ClienteExternoTr'
    trBody.style.backgroundColor = 'white'
    trBody.style.fontWeight = '400'  
    trBody.addEventListener('click',(e)=>{
      if(e.target.parentElement.value){
      ...
      let trList = document.getElementsByClassName('ClienteExternoTr') 
      for (let tr of trList){if(tr!=e.target.parentElement){
       setTimeout(()=>{tr.style.backgroundColor = 'rgb(222, 221, 221)'},50)} else {e.target.parentElement.style.backgroundColor = 'white'} } } )  
    ...
    //  let parsedOptionValue = JSON.parse(ClientesInternos_Necesidad.value)    
     props.backenData.current['ClienteExterno'].push({'Cliente':cliente.value,'Necesidad':necesidad.value,'Expectativa':expectativa.value,'elementHtml':trBody.innerHTML});
     props.summaryData.current['ClienteExterno'][trBody.innerHTML] = {'Cliente':cliente.value,'Necesidad':necesidad.value,'Expectativa':expectativa.value,'elementHtml':trBody.innerHTML}
     cliente.value = '';
     necesidad.value = ''
     expectativa.value = ''
    }
    ClienteExternoBody.appendChild(trBody)
    !ClientesExternosHead.children.length?ClientesExternosHead.appendChild(trHead):void 0
    updateDeleteRecordButtonStatus()
   }
   ...
   function handleRecordRemove(){
    ...
    if(Object.keys(selectedTableRecord.current).includes('recordToDeleteId')){
     props.backenData.current['recordsToDelete'].push({'ClienteExterno':selectedTableRecord.current['recordToDeleteId']})
     Object.keys(props.summaryData.current['recordsToDelete']).includes('ClienteExterno')? props.summaryData.current['recordsToDelete']['ClienteExterno'].push(selectedTableRecord.current['record']):props.summaryData.current['recordsToDelete']['ClienteExterno']=[selectedTableRecord.current['record']]   
    }else{
      if(selectedTableRecord.current['record']){
        Object.keys(props.summaryData.current['ClienteExterno']).includes(selectedTableRecord.current['record'].innerHTML)? (()=>{delete props.summaryData.current['ClienteExterno'][selectedTableRecord.current['record'].innerHTML]})():void 0      
       for(let counter=0;counter<=props.backenData.current['ClienteExterno'].length-1;counter++){
        let currentRecordToCreate = props.backenData.current['ClienteExterno'][counter]
        if(currentRecordToCreate['elementHtml']==selectedTableRecord.current['record'].innerHTML){props.backenData.current['ClienteExterno'].splice(counter,1)}
      }}}
