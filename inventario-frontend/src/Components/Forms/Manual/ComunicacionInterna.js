...
export default function ComunicacionInterna (props) {
 ...
 var selectedTableRecord = useRef(undefined);
  ...
  useEffect(() => {
    setTimeout(() => { 
      if(props.formsData.current['specificData'] && props.formsData.current['specificData']['ComunicacionInterna']) {
       let tHead = document.getElementsByClassName('ComunicacionInternaHead')[0]
       let tBody = document.getElementsByClassName('ComunicacionInternaBody')[0]
       for(let records of props.formsData.current['specificData']['ComunicacionInterna']) {
  ...
  function updateDeleteRecordButtonStatus() {
        let ComunicacionInternaBody = document.getElementsByClassName('ComunicacionInternaBody')[0]
        setDisplayDeleteRecordButton(ComunicacionInternaBody && ComunicacionInternaBody.children.length? true:false)
  }

  function HandleAdd() {
    let tipocomunicacion = document.getElementsByClassName('ComunicacionInterna_tipocomunicacion')[0]
    let periodicidad = document.getElementsByClassName('ComunicacionInterna_periodicidad')[0]   
    let ComunicacionInternaHead = document.getElementsByClassName('ComunicacionInternaHead')[0]    
    let ComunicacionInternaBody = document.getElementsByClassName('ComunicacionInternaBody')[0]    
    let data = [tipocomunicacion,periodicidad]
    let columns = ['Tipo de comunicación','Periodicidad']
    var trHead = document.createElement('tr')
    var trBody = document.createElement('tr')
    trBody.className = 'ComunicacionInternaTr'
    trBody.style.backgroundColor = 'white'
    trBody.style.fontWeight = '400'  
    trBody.addEventListener('click',(e)=>{
      if(e.target.parentElement.value){
      ...
      let trList = document.getElementsByClassName('ComunicacionInternaTr') 
      for (let tr of trList){if(tr!=e.target.parentElement){
       setTimeout(()=>{tr.style.backgroundColor = 'rgb(222, 221, 221)'},50)} else {e.target.parentElement.style.backgroundColor = 'white'} } } )  
    ...
    //  let parsedOptionValue = JSON.parse(ClientesInternos_Necesidad.value)    
     props.backenData.current['ComunicacionInterna'].push({'TipoComunicacion':tipocomunicacion.value,'Periodicidad':periodicidad.value,'elementHtml':trBody.innerHTML});
     props.summaryData.current['ComunicacionInterna'][trBody.innerHTML] = {'TipoComunicacion':tipocomunicacion.value,'Periodicidad':periodicidad.value,'elementHtml':trBody.innerHTML}
     tipocomunicacion.value = '';
     periodicidad.value = ''
    }
    ComunicacionInternaBody.appendChild(trBody)
    !ComunicacionInternaHead.children.length?ComunicacionInternaHead.appendChild(trHead):void 0
    updateDeleteRecordButtonStatus()
   }
   ...
   function handleRecordRemove(){
    ...
    if(Object.keys(selectedTableRecord.current).includes('recordToDeleteId')){
     props.backenData.current['recordsToDelete'].push({'ComunicacionInterna':selectedTableRecord.current['recordToDeleteId']})
     Object.keys(props.summaryData.current['recordsToDelete']).includes('ComunicacionInterna')? props.summaryData.current['recordsToDelete']['ComunicacionInterna'].push(selectedTableRecord.current['record']):props.summaryData.current['recordsToDelete']['ComunicacionInterna']=[selectedTableRecord.current['record']]   
    }else{
      if(selectedTableRecord.current['record']){
        Object.keys(props.summaryData.current['ComunicacionInterna']).includes(selectedTableRecord.current['record'].innerHTML)? (()=>{delete props.summaryData.current['ComunicacionInterna'][selectedTableRecord.current['record'].innerHTML]})():void 0      
       for(let counter=0;counter<=props.backenData.current['ComunicacionInterna'].length-1;counter++){
        let currentRecordToCreate = props.backenData.current['ComunicacionInterna'][counter]
        if(currentRecordToCreate['elementHtml']==selectedTableRecord.current['record'].innerHTML){props.backenData.current['ComunicacionInterna'].splice(counter,1)}
      }}}
 ...
 return (
  <div className="Sección_ComunicacionInterna">
   ...
   <br/>
   <br/>
   <table className='ComunicacionInternaTable' style={{'border':'0','borderCollapse':'separate'}}>
    <thead className='ComunicacionInternaHead' style={{'backgroundColor':'rgb(212, 208, 208)'}}></thead>
